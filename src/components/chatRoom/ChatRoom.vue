<template>
  ...
  <div class='chatroom'>
    <div class="friWrap">
      <div class="self-info">
        <span v-if="!this.$store.state.userInfo.icon"><img :src="require('@/assets/defaultIcon.png')" alt=""></span>
        ...
        <li v-for="(item, index) of curmessage" :key="index">
          <div class='itemsg' :class="item.from === id ? 'right' : 'left'">
            ...
            <p v-if="item.data.msg">
              ...
              <img :src="require(`@/assets/ee_${iconNumber(expressions.indexOf(item.data.msg) + 1)}.png`)" alt=""
                   v-if="expressions.includes(item.data.msg)">
            </p>
            <p v-if="item.data.id">
