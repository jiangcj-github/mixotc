      ...
      initData() { // 初始化信息
        this.WsProxy.send('otc','my_sales',{
          ...
          this.saleList.forEach(v => {
            v.price = typeof v.price == 'number' ? (v.price).formatFixed(2) : this.JsonBig.stringify(v.price).formatFixed(2)
            v.tradeable = typeof v.tradeable == 'number' ? v.tradeable.formatFixed(6) : this.JsonBig.stringify(v.tradeable).formatFixed(6)
            v.tradeable = (v.type == 2 && v.vary == 2) ? '不限量' : v.tradeable
            v.volume = typeof v.volume == 'number' ? v.volume.formatFixed(6) : this.JsonBig.stringify(v.volume).formatFixed(6)
            v.price_avg = typeof v.price_avg == 'number' ? v.price_avg.formatFixed(2) : this.JsonBig.stringify(v.price_avg).formatFixed(2)
          })
