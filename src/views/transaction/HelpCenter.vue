    ...
    <div class="clearfix">
      ...
      <div class="content-wrap">
        ...
        <div class="sale-wrap" v-if="showActive == 3">
          <h2>如何出售数字货币</h2>
          <span>在MIXOTC上您有两种方式可以出售数字币，一种是搜索广告，另一种是刊登出售数字币广告。</span>
        ...
        <div class="agreement-wrap" v-if="showActive == 4">
          <h2>用户协议</h2>
          <span>MIXOTC网络服务条款是由MIXOTC团队与MIXOTC用户就MIXOTC平台的各项服务所制定的相关权利义务规范。 MIXOTC的运营者依据本协议为用户提供服务。用户访问或使用本网站，即表示已接受并同意本协议的所有条件和条款，同意将其作为确定双方权利义务的依据；不愿接受本协议的，不得访问或使用本网站。本协议不涉及MIXOTC用户与其他用户之间因比特币交易而产生的法律关系及法律纠纷。 注意：MIXOTC有权对本协议进行修改，请用户定期查看本协议。</span>
          <h3>一、服务内容</h3>
          <ol>
        ...
        <div class="policy-wrap" v-if="showActive == 5">
          ...
          <h3>五、对我们的隐私政策的修改</h3>
          <p class="last-p">我们未来对我们的隐私政策的任何更改将会发布在此页面上， 我们将自行考虑， 也许会通过电子邮件通知您。 您使用我们的网站和服务的行为， 即表示您同意我们的隐私政策和声明。</p>
        </div>
      </div>
    </div>
    ...
    data() {
      return {
        ...
        serviceList: [
          ...
          '您有权在本网站查看会员账号信息，有权应用本网站提供的功能进行操作。',
        ...
        rightList: [
          ...
          '用户有权根据网站相关规定，获得MIXOTC给与的奖励（如手续费按比例返现等）',
        ...
        dutyList: [
          ...
          '用户不得通过任何手段恶意注册MIXOTC账号，包括但不限于以牟利、炒作、套现等为目的多个账号注册。用户亦不得盗用其他用户账号。 如用户违反上述规定，则MIXOTC有权直接采取一切必要的措施，包括但不限于删除用户发布的内容、取消用户在网站获得的星级、荣誉以及虚拟财富，暂停或查封用户账号，取消因违规所获利益，乃至通过诉讼形式追究用户法律责任等。',
          '禁止用户将MIXOTC以任何形式作为从事各种非法活动的场所、平台或媒介。未经MIXOTC的授权或许可，用户不得借用本站的名义从事任何商业活动，也不得以任何形式将MIXOTC作为从事商业活动的场所、平台或媒介。 如用户违反上述规定，则MIXOTC有权直接采取一切必要的措施，包括但不限于删除用户发布的内容、取消用户在网站获得的星级、荣誉以及虚拟财富，暂停或查封用户账号，取消因违规所获利益，乃至通过诉讼形式追究用户法律责任等。',
          '用户在MIXOTC以各种形式发布的一切信息，均应符合国家法律法规等相关规定及网站相关规定，符合社会公序良俗，并不侵犯任何第三方主体的合法权益，否则用户自行承担因此产生的一切法律后果，且MIXOTC因此受到的损失，有权向用户追偿。'
        ],
        refuseList: [
          'MIXOTC作为“网络服务提供者”的第三方平台，不担保网站平台上的信息及服务能充分满足用户的需求。对于用户在接受MIXOTC的服务过程中可能遇到的错误、侮辱、诽谤、不作为、淫秽、色情或亵渎事件，MIXOTC不承担法律责任。',
          '基于互联网的特殊性，MIXOTC也不担保服务不会受中断，对服务的及时性、安全性都不作担保，不承担非因MIXOTC导致的责任。 MIXOTC力图使用户能对本网站进行安全访问和使用，但MIXOTC不声明也不保证本网站或其服务器是不含病毒或其它潜在有害因素的；因此用户应使用业界公认的软件查杀任何自MIXOTC下载文件中的病毒。',
          '除非MIXOTC以书面形式明确约定，MIXOTC对于用户以任何方式（包括但不限于包含、经由、连接或下载）从本网站所获得的任何内容信息，包括但不限于广告等，不保证其淮确性、完整性、可靠性；对于用户因本网站上的内容信息而购买、获取的任何产品、服务、信息或资料，MIXOTC不承担责任。用户自行承担使用本网站信息内容所导致的风险。',
          'MIXOTC内所有用户所发表的用户评论，仅代表用户个人观点，并不表示本网站赞同其观点或证实其描述，本网站不承担用户评论引发的任何法律责任。',
        ...
        cookiesList: [
          ...
          'Cookies常用于记录访问者浏览我们网站上的各个项目时的习惯以及偏好。Cookies所搜集的资料是不记名的集体统计数据, 不载有个人资料。',
          'Cookies不能用于取得您的硬盘上的数据、您的电邮地址、及您的私人数据，其可使本网站或服务商系统识别到您的浏览器并捕捉和记忆信息。大多数浏览器都预设为可以接受Cookies。您可以选择将您的浏览器设定为不接受Cookies,或如果Cookies一被装上就通知您。不过, 若设定为禁止Cookies, 您或许便不能启动或使用我们网站的某些功能。'
        ],
        problemList: [
