        ...
        <div class="right">
          ...
          <div class="tb-tab">
            ...
            <p class="tb-tab-r" v-show="tab===1">
              <i>币币交易需要资产互转</i>
              <a :href="'https://qb.com?token='+token" target="_blank" class="btn green">币币交易</a>
            </p>
          </div>
          <!--法币账户-->
          ...
          <div class="bb" v-show="tab===1">
            ...
            <div v-else-if="bbErr===5">
              <div class="err empty">未开通币币账户，请先 <a :href="'https://qb.com?token='+token" target="_blank">开通</a></div>
            </div>
            <div v-else>
              <div class="err empty">无账户数据</div>
            </div>
  ...
  export default {
    ...
    methods:{
      async init(){
        ...
        let exData = await fetch(
          this.HostUrl.http+"api/v3/wallet/wallets/?user_id=" + this.JsonBig.stringify(userInfo.uid)
        ).then(res => res.json());
        ...
        if(!exData || !exData.user_id){
          ...
          if(exData.total_asset){
            this.balance2={
              numBtc:exData.total_asset.BTC,
              numCny: exData.total_asset.CNY
            };
          }
        }
        this.loadBb();
